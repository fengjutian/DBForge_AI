...
- **Text-to-SQL** — 自然语言描述需求，自动生成 MySQL 查询语句，支持 OpenAI / Groq / Claude / DeepSeek / Ollama
- **SQL 解释** — 对任意 SQL 语句进行自然语言解读，说明逻辑、涉及表字段及性能注意事项
- **查询优化** — 分析编辑器中的 SQL，给出索引建议、JOIN 改写、子查询优化等具体方案，并输出优化后的 SQL
- **错误诊断** — 粘贴 MySQL 报错信息，AI 定位根因并给出修复后的 SQL 及建议
- **安全审计** — 检测 SQL 注入风险、危险操作、权限越界等问题，按高 / 中 / 低危分级展示
- **Schema 文档生成** — 一键为当前连接的数据库生成 Markdown 格式技术文档，包含表用途、字段说明及关联关系
- **数据质量分析** — 对查询结果自动识别空值率、重复值、异常值、格式不一致等数据质量问题
- **结果洞察** — 对查询结果集进行自然语言摘要，提炼关键指标与异常数据
